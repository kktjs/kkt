...
module.exports = function loadExample({ example, projectName, projectPath }) {
  const cmds = [
    `mkdir -p ${projectName}`,
    `curl https://codeload.github.com/kktjs/kkt/tar.gz/master | tar -xz -C ${projectName} --strip=3 kkt-master/example/${example}`,
  ];
  ...
  return Promise.all(cmdPromises).then(async () => {
    ...
    const pkg = await axios.get('https://raw.githubusercontent.com/kktjs/kkt/master/package.json');
