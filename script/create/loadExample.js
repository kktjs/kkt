...
module.exports = function loadExample({ example, projectName, projectPath }) {
  const cmds = [
    `mkdir -p ${projectName}`,
    `curl https://codeload.github.com/jaywcjlove/kkt/tar.gz/master | tar -xz -C ${projectName} --strip=3 kkt-master/example/${example}`,
  ];
  ...
  return Promise.all(cmdPromises).then(async () => {
    ...
    const dependName = 'kkt';
    const stopKKTVersionSpinner = await output.wait(`settings dependencies on the ${output.cmd(dependName)} version.`);
    const pkg = await axios.get('https://raw.githubusercontent.com/jaywcjlove/kkt/master/package.json');
